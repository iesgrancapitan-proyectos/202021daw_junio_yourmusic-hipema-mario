...
def index_noticias(request):
    """ __lte """
    noticias = Noticias.objects.filter(created_at__lte=datetime.date.today()).values()
    '''
    ...
    for item in noticias:
        ...
        if user.count():
            ...
            user=UserProfileOjeadores.objects.filter(noticias=item['id']).values()
            for i in user:                 
                usuarios.append(i['nombre_profile'])
    ...
    return render(request, 'index_noticias.html', {'noticias':  noticias_page,'usuarios':usuarios})
